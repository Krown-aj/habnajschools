...
import { useGetClassById } from "@/hooks/useClasses";
...
const Class: React.FC<ClassProps> = ({
    ...
    // Fetch class data via React Query
    const {
        data: classData,
        isLoading,
        isFetching,
        isError,
        error,
    } = useGetClassById(classId, { enabled: !!classId });

    // show error toast once when fetch fails
    useEffect(() => {
        if (isError) {
            const msg = (error as any)?.message || "Could not fetch class data.";
            toast.current?.show({ severity: "error", summary: "Fetch Error", detail: msg, life: 4000 });
        }
    }, [isError, error]);
    ...
    return (
        <main className="min-h-screen bg-gradient-to-b from-white to-gray-50 p-4 sm:p-6 lg:p-12">
            <div className="max-w-7xl mx-auto">
                <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 sm:mb-8 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">{classData?.name ?? title}</h1>
                            <p className="text-xs sm:text-sm text-gray-500">
                                {subtitle ?? `Overview of ${classData?.name ?? "this class"} — Habnaj International Secondary Schools`}
                            </p>
                        </div>
                    </div>
                                    ...
                                    <div>
                                        <dl className="text-xs sm:text-sm text-gray-600 space-y-2 sm:space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <dd>
                                                    <Badge value={classData?._count?.students ?? (classData?.students?.length ?? 0)} severity="info" />
                                                </dd>
                                            </div>
                                        </dl>
                                    </div>
