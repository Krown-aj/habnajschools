...
const Grade: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const param = role === 'teacher' ? `teacherid=${session?.user.id}` : role === 'parent' ? `parentid=${session?.user.id}` : ''
        const [gradingResponse, classesResponse, subjectsResponse, studentsResponse] = await Promise.all([
          ...
          fetchWithErrorHandling(`/api/classes?${param}`, controller),
          fetchWithErrorHandling(`/api/subjects?${param}`, controller),
          fetchWithErrorHandling(`/api/students?${param}`, controller),
        ]);
  ...
  /* ---------------------------
     Sort assessments (CA1, CA2, CA3, Exams)
  --------------------------- */
  ...
  const columnDefs = useMemo<ColDef[]>(() => {
     ...
     */
    const assessmentColumns: ColDef[] = sortedAssessments.map((assessment) => ({
    ...
    return [
...

      {
        ...
        field: "serverTotal",
        valueGetter: (params) => {
          const computed = sortedAssessments.reduce(
...

      {
        ...
        valueGetter: (params) => {
          ...
          const score = sortedAssessments.reduce(
            (sum, a) => sum + (Number(params.data[a.id]) || 0),
            0
          );
  ...
  }, [filteredStudents, sortedAssessments, assessmentLookup, studentGradeLookup]);
