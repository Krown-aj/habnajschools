...
const Teacher: React.FC<TeacherProps> = () => {
        ...
        const fetchTeacherData = async () => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    toast.current?.show({ severity: "error", summary: "Fetch Error", detail: result.error || "Could not fetch teacher data." });
                }
            } catch (err: any) {
                toast.current?.show({ severity: "error", summary: "Fetch Error", detail: err.message || "Failed to fetch teacher data." });
            } finally {
                setLoading(false);
            }
        };
    ...
    // Resolve avatar source (supports static import fallback)
    const resolveImageSrc = (avarta: any) => {
        ...
        if (typeof avarta === "string") {
            ...
            // strip ./ or ../ then ensure leading slash
    ...
    const imageSrc = resolveImageSrc(teacherData?.avarta);
                                ...
                                <div>
                                    <dl className="text-xs sm:text-sm text-gray-600 space-y-2 sm:space-y-4">
                                        <div>
                                            ...
                                            <dd>{`${teacherData?.title || ""} ${teacherData?.firstname || ""} ${teacherData?.othername || ""} ${teacherData?.surname || ""}`.trim() || "–"}</dd>
                                        </div>
                                        <div>
                                    ...
                                    <dl className="text-xs sm:text-sm text-gray-600 space-y-2 sm:space-y-4">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div>
                                                ...
                                                <dd><Badge value={teacherData?._count?.subjects ?? 0} severity="info" /></dd>
                                            </div>
                                            <div>
                                                <dt className="font-semibold pb-3">Total Classes</dt>
                                                <dd><Badge value={teacherData?._count?.classes ?? 0} severity="info" /></dd>
                                            </div>
                                        </div>
                                    </dl>
                                </div>
